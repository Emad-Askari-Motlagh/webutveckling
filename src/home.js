...
function addProduct(user, item) {
  return Object.assign({}, user, { card: [{ name: "emad" }] });
}
function removeProduct(user, item) {
  return Object.assign({}, { card: {} });
}
function doCalculate(...args) {
  args.reduce(compose(addProduct, removeProduct));
}

doCalculate(user, { price: 24 });
